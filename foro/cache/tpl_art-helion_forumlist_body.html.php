<?php if (!defined('IN_PHPBB')) exit; $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if (( $_forumrow_val['S_IS_CAT'] && ! $_forumrow_val['S_FIRST_ROW'] ) || $_forumrow_val['S_NO_CAT']) {  ?>
	...
	<?php } if ($_forumrow_val['S_IS_CAT'] || $_forumrow_val['S_FIRST_ROW'] || $_forumrow_val['S_NO_CAT']) {  ?>
		...
		<div class="forabg">
			...
			<ul class="topiclist forums" data-id="<?php echo $_forumrow_val['FORUM_ID']; ?>">
			<?php $this->_tpldata['DEFINE']['.']['FIRST_ROW'] = TRUE; } if (! $_forumrow_val['S_IS_CAT']) {  ?>

		<li class="row row-outer<?php if ($_forumrow_val['S_UNREAD_FORUM']) {  ?> unread<?php } if ($_forumrow_val['S_LOCKED_FORUM']) {  ?> locked<?php } if ($this->_tpldata['DEFINE']['.']['FIRST_ROW']) {  ?> row1<?php } else { ?> row2<?php } ?>"><?php echo (isset($this->_tpldata['DEFINE']['.']['STYLE_ROW_START'])) ? $this->_tpldata['DEFINE']['.']['STYLE_ROW_START'] : ''; ?>

			<dl class="icon" style="background-image: url(<?php if ($_forumrow_val['FORUM_IMAGE']) {  echo $_forumrow_val['FORUM_IMAGE_SRC']; } else { echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; } ?>); background-repeat: no-repeat;">
				<dt title="<?php echo $_forumrow_val['FORUM_FOLDER_IMG_ALT']; ?>">
				<?php if ($this->_rootref['S_ENABLE_FEEDS'] && $_forumrow_val['S_FEED_ENABLED']) {  ?><a class="feed-icon-forum" title="<?php echo ((isset($this->_rootref['L_FEED'])) ? $this->_rootref['L_FEED'] : ((isset($user->lang['FEED'])) ? $user->lang['FEED'] : '{ FEED }')); ?> - <?php echo $_forumrow_val['FORUM_NAME']; ?>" href="<?php echo (isset($this->_rootref['U_FEED'])) ? $this->_rootref['U_FEED'] : ''; ?>?f=<?php echo $_forumrow_val['FORUM_ID']; ?>"><?php echo ((isset($this->_rootref['L_FEED'])) ? $this->_rootref['L_FEED'] : ((isset($user->lang['FEED'])) ? $user->lang['FEED'] : '{ FEED }')); ?> - <?php echo $_forumrow_val['FORUM_NAME']; ?></a><?php } ?>
					...
					<?php if ($_forumrow_val['MODERATORS']) {  ?>
						...
						<p class="moderators"><strong><?php echo $_forumrow_val['L_MODERATOR_STR']; ?>:</strong> <?php echo $_forumrow_val['MODERATORS']; ?></p>
					<?php } if ($_forumrow_val['SUBFORUMS'] && $_forumrow_val['S_LIST_SUBFORUMS']) {  ?><p class="subforums"><strong><?php echo $_forumrow_val['L_SUBFORUM_STR']; ?></strong> <?php echo $_forumrow_val['SUBFORUMS']; ?></p><?php } ?>
				...
				<?php } else if (! $_forumrow_val['S_IS_LINK']) {  ?>
					...
					<dd class="lastpost"><span>
						<?php if ($_forumrow_val['U_UNAPPROVED_TOPICS']) {  ?><a href="<?php echo $_forumrow_val['U_UNAPPROVED_TOPICS']; ?>"><?php echo (isset($this->_rootref['UNAPPROVED_IMG'])) ? $this->_rootref['UNAPPROVED_IMG'] : ''; ?></a><?php } if ($_forumrow_val['LAST_POST_TIME']) {  ?><dfn><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></dfn> <span class="by"><?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?></span> <?php echo $_forumrow_val['LAST_POSTER_FULL']; ?>
		...
		<?php if ($this->_tpldata['DEFINE']['.']['FIRST_ROW']) {  $this->_tpldata['DEFINE']['.']['FIRST_ROW'] = FALSE; } else { $this->_tpldata['DEFINE']['.']['FIRST_ROW'] = TRUE; } } if ($_forumrow_val['S_LAST_ROW']) {  ?>
